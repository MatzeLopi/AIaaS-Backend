...
async def upload_file(
    ...
    sql_schema = await schema_to_sqlschema(schema)

    table_name = await datasets.create_table_from_file(db, sql_schema, current_user)

    try:
        await datasets.insert_csv(
            db,
            table_name,
            file_path,
            schema,
        ...
        await datasets.delete_table(db, table_name, current_user)
        raise HTTPException(
